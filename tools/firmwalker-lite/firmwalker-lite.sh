...
for rel in /etc/passwd /etc/shadow /etc/group /etc/.htpasswd /www/.htpasswd /htdocs/.htpasswd; do
  ...
  sev=MEDIUM
  [[ "$rel" == */passwd || "$rel" == */shadow || "$rel" == */.htpasswd ]] && sev=HIGH

  emit credential "$sev" "$rel" \
    "credential/account file present: $(basename "$rel")"
done
...
for pattern in "${DEFAULT_CREDENTIAL_PATTERNS[@]}"; do
  while IFS= read -r p; do
    ...
    if grep -aIqiF -- "$pattern" "$p" 2>/dev/null; then
      r="$(relpath "$p")"
      echo "$pattern -> $r" >> "$FILE"
      emit credential_default HIGH "$r" \
        "default credential indicator: $pattern"
    fi
  done < "$TEXT_CANDIDATES"
done
...
for pattern in "${CREDENTIAL_ASSIGNMENT_PATTERNS[@]}"; do
  while IFS= read -r p; do
    ...
    if grep -aIqiF -- "$pattern" "$p" 2>/dev/null; then
      r="$(relpath "$p")"
      echo "$pattern -> $r" >> "$FILE"
      emit credential_assignment MEDIUM "$r" \
        "credential assignment indicator: $pattern"
    fi
  done < "$TEXT_CANDIDATES"
done
  ...
  while IFS= read -r -d '' p; do
    ...
    case "$b" in
      *.key|id_rsa|id_dsa|id_ecdsa|id_ed25519|ssh_host_*_key|dropbear_*_host_key)
      ...
      authorized_keys|known_hosts)
        sev=MEDIUM
        kind="SSH trust material"
        ;;
      *.pem|*.crt|*.cer|*.p12|*.pfx)
        sev=MEDIUM
        kind="certificate/TLS material"
        ;;
      *)
        continue
        ;;
    ...
    find "$d" -type f \
      \( -name '*.pem' -o -name '*.crt' -o -name '*.cer' -o -name '*.key' \
         ...
         -o -name 'id_ed25519' -o -name 'authorized_keys' \
         -o -name 'known_hosts' -o -name 'ssh_host_*_key' \
         -o -name 'dropbear_*_host_key' \) -print0 2>/dev/null
  )
...
for name in "${SERVICE_NAMES[@]}"; do
  while IFS= read -r d; do
    ...
    case "$name" in
      telnet|telnetd|ftp|ftpd|tftp|tftpd)
        sev=HIGH
        ;;
    esac
...
done < <(existing_dirs /etc/init.d /etc/rc.d /etc/config /etc/default) \
  | sort -u > "$STARTUP_CANDIDATES"
...
for name in "${SERVICE_NAMES[@]}"; do
  while IFS= read -r p; do
    ...
    if grep -aIqiE "(^|[/[:space:]])${name}([[:space:]]|$|[;&])" "$p" 2>/dev/null; then
      r="$(relpath "$p")"
      emit service INFO "$r" \
        "startup/config reference to service: $name"
    fi
...
#
# MODIFIED:
# Previously every .cgi/.php/.lua/.js/.html/.htm file below WEB_DIRS was
# automatically emitted as web_interface. This caused helper libraries,
# loaders and generic static files to be counted as Web/API interfaces.
#
# A file is now emitted only when:
#   1. its path itself strongly indicates a CGI/request handler, or
#   2. its contents contain a meaningful interface/API indicator.
#
# Generic http:// and https:// strings are intentionally excluded here.
# They are collected separately as network evidence.
# ---------------------------------------------------------------------------
...
while IFS= read -r d; do
  while IFS= read -r -d '' p; do
    ...
    # Strong path-based evidence:
    # CGI files/directories are request-handling components by design.
    case "$r" in
      /cgi-bin/*|/www/cgi-bin/*|/htdocs/cgi-bin/*)
        web_related=true
        web_reason="CGI/request-handler path"
        ;;
    esac

    # Content-based interface indicators.
    if [[ "$web_related" == false ]]; then
      for pattern in "${WEB_INTERFACE_PATTERNS[@]}"; do
        if grep -aIqiF -- "$pattern" "$p" 2>/dev/null; then
          web_related=true
          web_reason="interface indicator: $pattern"
          break
        fi
      done
    fi

    # Content-based API/request-handler indicators.
    if [[ "$web_related" == false ]]; then
      for pattern in "${WEB_ENDPOINT_PATTERNS[@]}"; do
        if grep -aIqiF -- "$pattern" "$p" 2>/dev/null; then
          web_related=true
          web_reason="API/request-handler indicator: $pattern"
          break
        fi
      done
    fi

    # Only meaningful Web/API candidates become web_interface evidence.
    if [[ "$web_related" == true ]]; then
      emit web_interface INFO "$r" \
        "web/API interface evidence: $web_reason"
    fi

    # Preserve command/exec markers under sensitive_pattern for current
    # correlator compatibility.
    for pattern in 'cmd=' 'exec=' 'command=' 'execute='; do
      if grep -aIqiF -- "$pattern" "$p" 2>/dev/null; then
        emit sensitive_pattern MEDIUM "$r" \
    ...
    find "$d" -type f \
      \( -name '*.cgi' -o -name '*.php' -o -name '*.lua' \
         -o -name '*.js' -o -name '*.html' -o -name '*.htm' \) \
      -print0 2>/dev/null
  )
...
while IFS= read -r d; do
  while IFS= read -r -d '' p; do
    ...
    # Bound text scanning to reasonably sized files; binaries are separately
    # represented as executables.
    ...
    for pattern in "${UPDATE_PATTERNS[@]}"; do
      if grep -aIqiF -- "$pattern" "$p" 2>/dev/null; then
        r="$(relpath "$p")"
        emit update INFO "$r" \
          "firmware-update indicator: $pattern"
        matched=true
      fi
    done
    ...
    find "$d" -maxdepth 4 -type f \
      \( -perm /111 -o -name '*.sh' -o -name '*.cgi' \
         -o -name '*.lua' -o -name '*.conf' -o -name '*.cfg' \) \
      -print0 2>/dev/null
  )
...
COMPONENTS=(
  busybox openssl dropbear ssh sshd dnsmasq lighttpd
  nginx httpd uhttpd curl wget
)
...
for name in "${COMPONENTS[@]}"; do
  while IFS= read -r d; do
    ...
    if command -v strings >/dev/null 2>&1; then
      ver="$(strings -a "$p" 2>/dev/null | \
        grep -Eim1 '(BusyBox v|OpenSSL [0-9]|Dropbear_[0-9]|dropbear [0-9]|OpenSSH[_ ]|dnsmasq[- ]?[0-9]|lighttpd[/ ]|nginx[/ ]|curl [0-9]|Wget[/ ]|uClibc|GLIBC_[0-9])' \
        || true)"
    ...
    if [[ -n "$ver" ]]; then
      emit component INFO "$r" \
        "component: $name; version hint: $ver"
    else
      emit component INFO "$r" \
...
for glob in \
  "$FIRMDIR"/lib/libc.so.* \
  "$FIRMDIR"/lib/libssl.so.* \
  "$FIRMDIR"/usr/lib/libssl.so.*
do
  [[ -f "$glob" ]] || continue

  r="$(relpath "$glob")"
  emit component INFO "$r" \
    "shared component present: $(basename "$glob")"
done
...
#
# MODIFIED:
# A weak algorithm name by itself remains INFO evidence only.
# crypto_weak_context is emitted only when the algorithm appears close to
# a context that is meaningful for that algorithm family.
#
# This prevents generic configuration/package files from becoming F-08
# simply because broad words such as auth/checksum/integrity appear nearby.
# ---------------------------------------------------------------------------
...
for pattern in "${WEAK_CRYPTO[@]}"; do
  while IFS= read -r p; do
    ...
    # A weak/deprecated algorithm string alone is supporting evidence only.
    emit crypto_weak_indicator INFO "$r" \
      "legacy/weak crypto indicator: $pattern"

    while IFS=: read -r lineno _; do
      [[ "$lineno" =~ ^[0-9]+$ ]] || continue

      # Keep the context local to reduce accidental association.
      start=$((lineno > 2 ? lineno - 2 : 1))
      end=$((lineno + 2))
      snippet="$(sed -n "${start},${end}p" "$p" 2>/dev/null)"
      ...
      case "$pattern" in
        ...
        MD5|SHA1|SHA-1)
          # Hash algorithms become security-relevant only when nearby text
          # suggests credential hashing, signatures or verification.
          for ctx in \
            'password' \
            'passwd' \
            'signature' \
            ...
            'hash algorithm'
          do
            if grep -aIqiF -- "$ctx" <<< "$snippet"; then
              context="$ctx"
              break
            fi
          done
        ...
        DES|3DES|RC4|ECB)
          # Cipher/mode indicators require encryption/key-use context.
          for ctx in \
            'encrypt' \
            'encryption' \
            'decrypt' \
      ...
      if [[ -n "$context" ]]; then
        emit crypto_weak_context MEDIUM "$r" \
          "weak crypto indicator with security-relevant use context: $pattern; context: $context"
        break
      fi
...
# Modern cryptographic algorithm indicators.
for pattern in "${MODERN_CRYPTO[@]}"; do
  while IFS= read -r p; do
    [[ -f "$p" ]] || continue

    if grep -aIqiF -- "$pattern" "$p" 2>/dev/null; then
      emit crypto INFO "$(relpath "$p")" \
        "modern crypto indicator: $pattern"
    fi
...
for pattern in "${ENCODING_PATTERNS[@]}"; do
  while IFS= read -r p; do
    ...
    if grep -aIqiF -- "$pattern" "$p" 2>/dev/null; then
      emit encoding INFO "$(relpath "$p")" \
        "encoding/obfuscation indicator: $pattern"
    fi
...
while IFS= read -r p; do
  ...
  while IFS= read -r ip; do
    [[ -n "$ip" ]] && \
      emit network INFO "$(relpath "$p")" "IP address: $ip"
  done < <(
    grep -aEho '\b(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b' \
      "$p" 2>/dev/null | sort -u || true
  )

  while IFS= read -r url; do
    [[ -n "$url" ]] && \
      emit network INFO "$(relpath "$p")" "URL: $url"
  done < <(
    grep -aEoh '(http|https)://[^/"[:space:]]+' \
      "$p" 2>/dev/null | sort -u || true
  )
