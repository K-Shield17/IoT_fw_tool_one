...
def loc($a): ($a|map(.asset)|map(select(.!="" and .!="-"))|unique) as $x | if ($x|length)==0 then "-" elif ($x|length)==1 then $x[0] else $x[0]+" 외 "+((($x|length)-1)|tostring)+"개" end;
...
def kc($r;$x):
  if $r=="F-01" then "해당 서비스의 실제 활성화·외부 노출 여부와 메모리 손상 취약점 존재 여부를 추가 확인"
  elif $r=="F-02" then "키의 실제 사용 서비스, 장비별 고유성 및 펌웨어 외부 유출 가능성을 확인"
  elif $r=="F-03" then "Credential store 접근 가능성, 계정 사용 범위 및 오프라인 크래킹 영향을 확인"
  elif $r=="F-04" then "실제 장비에서 Telnet 서비스 활성화·외부 노출·인증 정책을 확인"
  elif $r=="F-05" then "사용자 입력이 OS 명령 실행 sink까지 도달하는 Source-to-Sink 흐름을 확인"
  elif $r=="F-06" then "펌웨어 빌드 정책과 Toolchain Hardening 설정을 확인"
  elif $r=="F-07" then "업데이트 이미지의 서명·무결성 검증이 실제 업데이트 경로에서 강제되는지 확인"
  elif $r=="F-08" then "정확한 구성요소 버전을 식별하고 CVE 및 벤더 보안 공지와 대조"
  else $x end;

def ki($r;$a):
  if $r=="F-01" then ["실제 메모리 손상 취약점이 존재할 경우 exploit resistance가 낮아질 수 있음"]
  elif $r=="F-02" then ["민감 키가 비인가 사용자 또는 프로세스에 노출될 경우 인증 우회·서비스 위장·키 재사용 위험이 증가할 수 있음"]
...
def krem($r;$a):
  if $r=="F-01" then ["네트워크 서비스 ELF에 누락된 Canary, NX, PIE, Full RELRO 등 적용 가능한 보호기법을 활성화"]
  elif $r=="F-02" then ["민감 키 파일 권한을 최소화하고 서비스 계정만 읽을 수 있도록 접근권한을 제한"]
  elif $r=="F-03" then ["Unix-MD5 대신 충분한 비용의 현대적 비밀번호 해시 알고리즘과 고유 salt를 사용"]
  elif $r=="F-04" then ["Telnet을 비활성화하고 필요한 원격 관리 기능은 SSH 등 암호화된 프로토콜로 전환"]
  elif $r=="F-05" then ["외부 입력을 명령 문자열에 직접 결합하지 말고 허용목록 기반 검증과 안전한 API를 사용"]
  elif $r=="F-06" then ["빌드 시스템과 Toolchain에 공통 Hardening 정책을 적용하고 릴리스 빌드에서 적용 여부를 검증"]
  elif $r=="F-07" then ["업데이트 패키지에 전자서명 기반 출처 검증과 무결성 검사를 적용하고 검증 실패 시 설치를 차단"]
  elif $r=="F-08" then ["구성요소 버전을 식별·관리하고 알려진 취약점이 없는 지원 버전으로 업데이트"]
  else $a end;

def kb($r;$e):
  if $r=="F-01" then
    (($e|map(select(test("Role:|Startup/config reference:|Weak mitigation count:|RELRO:|Stack Canary:|NX:|PIE:|FORTIFY:"))) | unique) as $x |
    if ($x|length)>0 then ($x|join("; ")) else "네트워크 서비스 ELF의 Hardening profile과 서비스 Context를 상관분석하여 Finding 조건을 확인" end)
  elif $r=="F-02" then "Crypto/SSH 관련 민감 키 자료와 파일 권한 정보를 결합하여 group 또는 other 읽기 권한을 확인"
  elif $r=="F-03" then "Credential 관련 Evidence에서 Unix-MD5 password hash 패턴을 확인"
  elif $r=="F-04" then "Telnet 서비스 구성요소와 Startup/Config reference를 함께 확인"
  elif $r=="F-05" then "Web 서비스 존재와 Web 경로의 명령 실행 관련 sensitive pattern을 함께 확인"
  elif $r=="F-07" then "Firmware Update 관련 Evidence와 업데이트 검증 조건을 상관분석하여 추가 검증이 필요한 패턴을 확인"
  elif $r=="F-08" then "구성요소·버전 관련 Evidence를 상관분석하여 알려진 취약 버전 여부를 추가 확인해야 하는 패턴을 확인"
  else if ($e|length)>0 then ($e|join("; ")) else "여러 정적 분석 관찰값의 상관분석을 통해 Finding 조건을 확인" end end;

def case_rem($r;$e;$a):
  if $r=="F-01" then
    ([if any($e[]?;test("No Canary")) then "Stack Canary 적용" else empty end,
      if any($e[]?;test("PIE Disabled")) then "PIE 적용" else empty end,
      if any($e[]?;test("No RELRO|Partial RELRO")) then "Full RELRO 적용" else empty end,
      if any($e[]?;test("NX disabled|NX Disabled")) then "NX 활성화" else empty end,
      if any($e[]?;test("RPATH.*(unsafe|enabled)|RUNPATH.*(unsafe|enabled)";"i")) then "불필요하거나 안전하지 않은 RPATH/RUNPATH 제거" else empty end] | unique)
  elif $r=="F-02" then ["민감 키 파일의 읽기 권한을 최소화하고 실제 사용 서비스 계정만 접근하도록 제한"]
  elif $r=="F-03" then ["Unix-MD5를 현대적인 비밀번호 해시 방식으로 교체하고 영향받는 Credential을 갱신"]
  elif $r=="F-04" then ["해당 Telnet 구성요소의 실제 사용 여부를 확인하고 불필요한 경우 비활성화","원격 관리가 필요한 경우 SSH 등 암호화된 프로토콜로 전환"]
  elif $r=="F-05" then ["해당 위치의 사용자 입력과 명령 실행 함수 사이의 흐름을 검토하고 안전한 API 및 입력 검증을 적용"]
  else krem($r;$a) end;

def case_check($r;$e):
  if $r=="F-01" then
    (if any($e[]?;test("Startup/config reference: false")) then
      "Startup/Config 참조가 확인되지 않았으므로 실제 실행 여부를 우선 확인하고, 활성화된 경우 외부 노출 및 메모리 손상 취약점 존재 여부를 추가 확인"
     else "해당 서비스의 실제 활성화·외부 노출 여부와 메모리 손상 취약점 존재 여부를 추가 확인" end)
  else kc($r;"") end;
...
def supp($type;$title;$result;$check;$rem):
  ...
  if ($a|length)==0 then empty else {
    ...
    analysis:$result,additional_check:$check,impact:[],remediation:[$rem],cases:[]
  } end;

($c[0]//{security_cases:[],systemic_findings:[],informational:[]}) as $r |

([$r.security_cases[]? | (.evidence//[]) as $e | {
  ...
  basis:kb((.rule_id//"");(($e+(.confidence_basis//[]))|map(tostring))),
  analysis:kr((.rule_id//"");(.analysis//"")),additional_check:case_check((.rule_id//"");(($e+(.confidence_basis//[]))|map(tostring))),
  impact:ki((.rule_id//"");(.potential_impact//[])),remediation:case_rem((.rule_id//"");(($e+(.confidence_basis//[]))|map(tostring));(.remediation//[])),
  raw_evidence:(($e+(.confidence_basis//[]))|map(tostring))
}]) as $security_cases |

($security_cases | sort_by(.rule_id) | group_by(.rule_id) | map(
  . as $g | {
    kind:"security_group",rule_id:$g[0].rule_id,finding_status:(if any($g[];.finding_status=="IDENTIFIED") then "IDENTIFIED" elif any($g[];.finding_status=="POTENTIAL") then "POTENTIAL" else $g[0].finding_status end),
    title:$g[0].title,severity:maxsev($g),confidence:maxconf($g),asset:loc($g),locations:($g|map(.asset)|unique),
    category:$g[0].category,
    result:(if ($g|length)>1 then ($g[0].result+" 영향 위치 "+(($g|length)|tostring)+"개가 확인됨.") else $g[0].result end),
    basis:"동일 Finding Rule에 해당하는 위치 "+(($g|length)|tostring)+"개를 통합하여 요약함",
    analysis:$g[0].analysis,additional_check:kc($g[0].rule_id;$g[0].additional_check),
    impact:($g|map(.impact[])|unique),remediation:($g|map(.remediation[])|unique),cases:$g
  }
)) as $security |
...
([$r.systemic_findings[]? | {
  ...
  asset:"Firmware-wide",locations:["Firmware-wide"],category:(.category//"systemic"),result:kr((.rule_id//"F-06");(.analysis//"")),
  basis:((.analysis_scope//{}) as $x |
    ...
    if ($total!=null and $affected!=null) then "분석 ELF "+($total|tostring)+"개 중 "+($affected|tostring)+"개"+(if $pct!=null then " ("+($pct|tostring)+"%)" else "" end)+"에서 반복 패턴 확인"
    else "다수 바이너리에서 반복되는 보안 설정 패턴 확인" end),
  analysis:kr((.rule_id//"F-06");(.analysis//"")),additional_check:kc((.rule_id//"F-06");""),
  impact:ki((.rule_id//"F-06");(.potential_impact//[])),remediation:krem((.rule_id//"F-06");(.remediation//[])),cases:[]
}]) as $systemic |
...
def casecount: if ((.cases//[])|length)>0 then (.cases|length) else 1 end;
...

{
  ...
  summary:{
    ...
    risk_components:{severity:($severity_score|round),confidence:($confidence_score|round),exposure:($exposure_score|round),prevalence:($prevalence_score|round)},
...
.finding.high{border-left-color:var(--high)}.finding.medium{border-left-color:var(--med)}.case{border-top:1px solid #dbe2ea;margin-top:14px;padding-top:4px}.badge{display:inline-block;padding:4px 9px;border-radius:999px;font-weight:700}.found{background:#fee4e2;color:#912018}.potential{background:#fef0c7;color:#93370d}.partial{background:#e0f2fe;color:#075985}
...
def cases($f):
  if (($f.cases//[])|length)==0 then
    "<table><tr><th style=\"width:22%\">위치</th><td>"+code($f.asset)+"</td></tr>"+
    "<tr><th>점검 결과</th>"+td($f.result)+"</tr><tr><th>확인 근거 요약</th>"+td($f.basis)+"</tr>"+
    "<tr><th>보안 영향</th>"+(if (($f.impact//[])|length)>0 then "<td>"+bullets($f.impact)+"</td>" else td($f.analysis//"") end)+"</tr>"+
    "<tr><th>추가 확인</th>"+td($f.additional_check//"")+"</tr><tr><th>조치 방안</th>"+(if (($f.remediation//[])|length)>0 then "<td>"+bullets($f.remediation)+"</td>" else td("-") end)+"</tr></table>"
  else
    ([$f.cases[] |
      "<div class=\"case\"><h4>"+code(.asset)+"</h4><table>"+
      "<tr><th style=\"width:22%\">점검 결과</th>"+td(.result)+"</tr>"+
      "<tr><th>확인 근거 요약</th>"+td(.basis)+"</tr>"+
      "<tr><th>보안 영향</th>"+(if ((.impact//[])|length)>0 then "<td>"+bullets(.impact)+"</td>" else td(.analysis//"") end)+"</tr>"+
      "<tr><th>추가 확인</th>"+td(.additional_check//"")+"</tr>"+
      "<tr><th>조치 방안</th>"+(if ((.remediation//[])|length)>0 then "<td>"+bullets(.remediation)+"</td>" else td("-") end)+"</tr></table></div>"
    ]|join(""))
  end;
...
"<tr>"+td("High")+td("복수의 유의미한 보안 문제가 확인되고 공격 표면 또는 영향 범위가 높은 상태")+"</tr>"+
"<tr>"+td("Medium")+td("보안 문제가 확인되었으나 실제 악용을 위해 추가 조건 또는 검증이 필요한 상태")+"</tr>"+
"<tr>"+td("Low")+td("즉각적인 악용 가능성은 낮으나 보안 개선이 필요한 상태")+"</tr>"+
...
"<h2>7. 진단 상세</h2><p class=\"subtitle\">동일 Finding은 하나의 진단 항목으로 관리하되, 각 위치에서 실제 확인된 Evidence와 Context를 기준으로 대상별 상세 결과를 구분하여 제공한다.</p>"+
...
"<section class=\"finding "+$c+"\"><h3>"+(.id|e)+" · "+(.title|e)+" ["+(.severity|e)+"]</h3>"+cases(.)+"</section>"]|join("")) end)+
