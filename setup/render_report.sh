...
def detail_key:
  [(.role//""),(.problems//[]|sort|join("|")),(.normal//[]|sort|join("|")),(.context//""),
   (.judgment//""),(.impact//[]|sort|join("|")),(.additional_check//""),(.remediation//[]|sort|join("|"))] | join("###");
...
def remitem($r;$severity;$text):
  if any($findings[]?;.rule_id==$r) then {rule_id:$r,severity:$severity,recommendation:$text} else empty end;

([
  remitem("F-02";([$findings[]|select(.rule_id=="F-02")|.severity]|first//"HIGH");"민감 Key 파일은 실제 사용하는 서비스 계정만 접근할 수 있도록 파일 권한을 최소화하고, 공유 Key가 확인될 경우 장비별 고유 Key로 교체"),
  remitem("F-03";([$findings[]|select(.rule_id=="F-03")|.severity]|first//"HIGH");"Unix-MD5 기반 비밀번호 저장 방식을 안전한 비밀번호 해시 방식으로 변경하고 영향받는 Credential 교체"),
  remitem("F-04";([$findings[]|select(.rule_id=="F-04")|.severity]|first//"HIGH");"Telnet 원격 서비스를 비활성화하고 필요한 원격 관리 기능은 SSH 등 암호화된 프로토콜로 전환"),
  remitem("F-05";([$findings[]|select(.rule_id=="F-05")|.severity]|first//"MEDIUM");"Web 입력과 명령 실행 함수 사이의 Source-to-Sink 흐름을 검토하고 외부 입력 검증 및 안전한 API 사용"),
  remitem("F-01";([$findings[]|select(.rule_id=="F-01")|.severity]|first//"MEDIUM");"네트워크 서비스 바이너리에 실제 누락된 Stack Canary, PIE, Full RELRO, NX 등의 보호기법 적용"),
  remitem("F-06";([$findings[]|select(.rule_id=="F-06")|.severity]|first//"MEDIUM");"반복적인 Binary Hardening 미흡을 방지하도록 Firmware Toolchain 및 릴리스 빌드에 공통 Hardening 정책 적용"),
  remitem("F-07";([$findings[]|select(.rule_id=="F-07")|.severity]|first//"MEDIUM");"Firmware Update에 전자서명 기반 출처 검증과 무결성 검사를 적용하고 검증 실패 시 설치 차단"),
  remitem("F-08";([$findings[]|select(.rule_id=="F-08")|.severity]|first//"MEDIUM");"구성요소 Version을 관리하고 알려진 취약점이 확인된 구성요소는 지원되는 보안 Version으로 업데이트")
] | unique_by(.recommendation) | sort_by([-(.severity|sev),.rule_id])) as $rem |
...
def details($f):
  if (($f.detail_groups//[])|length)==0 then
    ...
    ([$f.detail_groups[] |
      ...
      "<tr><th style=\"width:22%\">위치</th><td>"+paths(.assets)+"</td></tr>"+
      "<tr><th>자산 역할</th>"+td(.role//"-")+"</tr>"+
...
else ([.remediation[]|"<tr><td class=\""+(.severity|e)+"\">"+(.severity|e)+"</td>"+td(.recommendation)+"</tr>"]|join("")) end)+"</table>"+
